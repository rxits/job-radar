...
- All SQL lives in `src/lib/db.ts`. Sources implement `JobSource` in `src/lib/sources/` and register in `sources/index.ts`.
- Tests: `pnpm test` (vitest, fixtures in `test/fixtures/`, no live network in tests). Typecheck: `pnpm exec tsc --noEmit`.
- CLI: `pnpm scrape`, `pnpm match`. Dashboard: `pnpm dev` → localhost:3000.
